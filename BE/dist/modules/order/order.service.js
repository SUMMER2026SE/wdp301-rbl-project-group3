...
class OrderService {
    ...
    async placeOrder(customerId, data) {
        ...
        const minOrderSetting = await system_setting_repository_1.systemSettingRepository.findByKey('min_order_amount');
        const minOrderVal = Number(minOrderSetting?.value ?? 0);
        if (totalAmountBeforeDiscount < minOrderVal) {
            throw new errorHandler_middleware_1.AppError(`Giá trị đơn hàng tối thiểu phải từ ${new Intl.NumberFormat('vi-VN', {
                style: 'currency',
                currency: 'VND',
            }).format(minOrderVal)} trở lên.`, 400);
        }
        ...
        const [freeShippingThresholdSetting, defaultDeliveryFeeSetting, vatRateSetting] = await Promise.all([
            system_setting_repository_1.systemSettingRepository.findByKey('free_shipping_threshold'),
            system_setting_repository_1.systemSettingRepository.findByKey('default_delivery_fee'),
            system_setting_repository_1.systemSettingRepository.findByKey('vat_rate')
        ]);
