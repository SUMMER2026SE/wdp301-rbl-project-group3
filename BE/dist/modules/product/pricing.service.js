...
`;
    ...
    async suggestPriceBulk(items) {
        ...
        for (const item of items) {
            ...
            const category = await category_model_1.Category.findById(categoryId).exec();
            if (!category) {
                throw new errorHandler_middleware_1.AppError('Không tìm thấy danh mục cho sản phẩm: ' + (name || sku || ''), 404);
            }
            const minMargin = category.minMargin || 0;
            const floorPrice = Math.round(costPrice * (1 + minMargin / 100));
            // Fetch stock
...
`;
        ...
        if (!Array.isArray(resultsList)) {
            throw new errorHandler_middleware_1.AppError('AI định giá trả về sai định dạng danh sách.', 500);
        }
        const finalResults = [];
