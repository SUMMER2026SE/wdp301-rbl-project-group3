...
function buildInvoiceLines(invoice: IInvoice): string[] {
  const listedAmount =
    invoice.listedAmount ??
    invoice.items.reduce((sum, item) => sum + item.subtotal, 0);
  const discountAmount =
    invoice.discountAmount ?? Math.max(0, listedAmount - invoice.totalAmount);
  const lines = [
  ...
  lines.push(
    '',
    `TAM TINH: ${money(listedAmount)}`,
    `GIAM GIA: ${money(discountAmount)}`,
    `TONG CONG: ${money(invoice.totalAmount)}`
  );
  return lines.map(ascii);
}

export function createInvoicePdf(invoice: IInvoice): Buffer {
